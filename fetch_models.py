#!/usr/bin/env python3
"""Fetch all models from OpenRouter and generate a comparison HTML page."""
...
def fetch_models():
    ...
    req = urllib.request.Request(url, headers={"User-Agent": "OpenRouter-Models-Fetcher/1.0"})
...
def process_models(raw_models):
    """Process raw model data into a clean list for display."""
    models = []
    for m in raw_models:
        ...
        reasoning_price = float(pricing.get("internal_reasoning", "0") or "0")
        ...
        models.append({
            ...
            "max_completion": top.get("max_completion_tokens", 0) or 0,
            ...
            # Price per 1M tokens for display
...
def generate_html(models, fetch_time, flashship_url="/flashship"):
    ...
    all_output_mods = sorted(set(mod for m in models for mod in m["output_modalities"]))
    ...
    output_mods_json = json.dumps(all_output_mods, ensure_ascii=False)
...
const OUTPUT_MODS = {output_mods_json};
...
const I18N = {{
  en: {{
    ...
    cmp_name: 'Model Name',
    cmp_id: 'Model ID',
    cmp_provider: 'Provider',
    cmp_desc: 'Description',
    cmp_input_price: 'Input Price / 1M tokens',
    cmp_output_price: 'Output Price / 1M tokens',
    cmp_reasoning_price: 'Reasoning Price / 1M tokens',
    cmp_context: 'Context Length',
    cmp_max_output: 'Max Output Tokens',
    cmp_input_mod: 'Input Modalities',
    cmp_output_mod: 'Output Modalities',
    cmp_params: 'Supported Parameters',
  ...
  zh: {{
    ...
    cmp_provider: '\u670d\u52a1\u5546',
    cmp_desc: '\u63cf\u8ff0',
    cmp_input_price: '\u8f93\u5165\u4ef7\u683c / 1M tokens',
    cmp_output_price: '\u8f93\u51fa\u4ef7\u683c / 1M tokens',
    cmp_reasoning_price: '\u63a8\u7406\u4ef7\u683c / 1M tokens',
    cmp_context: '\u4e0a\u4e0b\u6587\u957f\u5ea6',
    cmp_max_output: '\u6700\u5927\u8f93\u51fa Tokens',
    cmp_input_mod: '\u8f93\u5165\u6a21\u6001',
    cmp_output_mod: '\u8f93\u51fa\u6a21\u6001',
    cmp_params: '\u652f\u6301\u53c2\u6570',
    selected_suffix: '\u4e2a\u5df2\u9009',
    ...
    <script>
      ...
      function renderFlashshipSort() {{
        ...
        rows.sort((rowA, rowB) => {{
          ...
          const header = sortHeaders.find(th => th.dataset.sort === sortKey);
          const type = header ? header.dataset.sortType : 'text';
          const dataKey = sortKey.replace(/-([a-z])/g, (_, c) => c.toUpperCase());
          const a = rowA.dataset[dataKey] || '';
          const b = rowB.dataset[dataKey] || '';
...
def main():
    ...
    models = fetch_models()
    print(f"  Fetched {len(models)} raw models")

    processed = process_models(models)
    print(f"  Processed {len(processed)} models")

    fetch_time = datetime.now()
    explorer_filename = f"output/models_{fetch_time.strftime('%Y%m%d_%H%M%S')}.html"
    flashship_filename = f"output/flashship_{fetch_time.strftime('%Y%m%d_%H%M%S')}.html"

    html = generate_html(
        processed,
        fetch_time,
        flashship_url=os.path.basename(flashship_filename),
    )
    flashship_html = generate_flashship_html(
        processed,
        fetch_time,
        explorer_url=os.path.basename(explorer_filename),
        flashship_url=os.path.basename(flashship_filename),
    )

    os.makedirs("output", exist_ok=True)
    with open(explorer_filename, "w", encoding="utf-8") as f:
        f.write(html)
    with open(flashship_filename, "w", encoding="utf-8") as f:
        f.write(flashship_html)

    print(f"  Generated explorer: {explorer_filename}")
    print(f"  Generated flashship: {flashship_filename}")
    print("Done!")
    return explorer_filename, flashship_filename
